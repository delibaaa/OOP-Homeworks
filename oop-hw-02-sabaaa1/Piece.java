...
public class Piece {
	...
	public Piece(TPoint[] points) {
		...
		for(TPoint p : this.body) {
			if(p.x + 1 > currMaxWidth) currMaxWidth = p.x + 1;
			if(p.y + 1 > currMaxHeight) currMaxHeight = p.y + 1;
		}
		this.width = currMaxWidth;
		this.height = currMaxHeight;

		// SKIRT
		this.skirt = new int[this.width];
		for(int i = 1 ; i <= this.width; i++) {
			this.skirt[i - 1 ] = (forThisIndex(i));
		}
		this.next = null;
	}

	private int forThisIndex(int i) {
		for(TPoint p : this.body) {
			if(p.x == i-1) return p.y;
		}
		return -1 ;
	}
	...
	public Piece computeNextRotation() {
		...
		for(int i = 0 ; i < this.body.length ; i++) {
			int newX = height - 1 - this.body[i].y;
			int newY = this.body[i].x;
			newPoints[i] = new TPoint(newX, newY);
		}
	...
	public boolean equals(Object obj) {
		...
		for (int i = 0; i < this.body.length; i++) {
			if (!this.body[i].equals(other.body[i])) return false;
		}
		return true;
	}
